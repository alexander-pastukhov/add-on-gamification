...
bootObserverAverage <- function(values){
  observerAverage= boot(data= values, statistic = meanFn, R= 1000)
  data.frame(Average=observerAverage$t[, 1])
}

bootGroupMean <- boot_results %>%
  group_by(Observer, Condition, COA, coaBlockIndex) %>%
  do(bootObserverPerformance(.$Correct))

groupCI <- boot(data= seq(1, max(boot_results$iTrial)), statistic= computeGroupMean, R= 1000)

boot.ci(groupCI, conf = 0.95, type = "bca")
